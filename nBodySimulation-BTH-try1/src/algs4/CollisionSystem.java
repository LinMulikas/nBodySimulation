...
public class CollisionSystem {
    ...
    private void calculateNext(Particle a, Particle[] particles, double dt) {
        if (a == null) {
            return;
        }

        a.calNetForce(particles);
        a.calAcceleration();
    ...
    //运用BHT计算所有点的合外力
    public void addForce(Particle[] particles) {
        BHT tree = new BHT(q);
        //将所有点插入树
        for (Particle particle : particles) {
...
//				//计算崭新的加速状态
//				for (Particle particle : particles) {
//					particle.calNetForce(particles);
//					particle.calAcceleration();
//				}
